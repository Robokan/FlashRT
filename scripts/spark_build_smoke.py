...
def main() -> int:
    ...
    #    (batch=2 for CFG, num_heads=8, head_dim=256, seqlen=1024)
    #    chosen to stress the same code path as Pi0.5 inference.
    try:
        import torch
        ...
        # FA2 BHSD layout: (batch, num_heads, seqlen, head_dim)
        B, H, S, D = 2, 8, 1024, 256
        torch.manual_seed(0)
        ...
        out = flash_rt_fa2.fwd_bf16(q, k, v)
        torch.cuda.synchronize()

        # FA2 wrappers in this repo return either a tensor or a list-like
        # of tensors depending on causal/split selection; handle both.
        if isinstance(out, (list, tuple)):
            out = out[0]

        if not torch.is_tensor(out):
            _fail("fa2.fwd_bf16 dtype", f"got {type(out).__name__}, expected Tensor")
            failures.append("fa2_kernel")
        elif not torch.isfinite(out).all():
            n_nan = int(torch.isnan(out).sum())
            n_inf = int(torch.isinf(out).sum())
            _fail("fa2.fwd_bf16 finite", f"got {n_nan} NaN, {n_inf} Inf")
            failures.append("fa2_kernel")
        elif out.shape != (B, H, S, D):
            _fail("fa2.fwd_bf16 shape", f"got {tuple(out.shape)}, expected {(B, H, S, D)}")
            failures.append("fa2_kernel")
        else:
            _pass("fa2.fwd_bf16 launch", f"shape={tuple(out.shape)} dtype={out.dtype}")
    except Exception as e:  # pragma: no cover
